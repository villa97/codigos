		...
		cin>>seleccion;
			...
			switch(seleccion)
			{
				...
				case 3:
					...
					fin=1;//el fin lo igualo a 1 para que cuando sea negativo valdra 0 y se terminara de ejecutar el proceso
					total=0;//el total lo igualo a cero para no tener basura dentro de el
					while(fin!=0)//digo que while se ejecutara mientras que la variable fin  no sea cero
					{
						cout<<"Calificacion:";//pido la calificacion
						cin>>cal;//la almaceno
						if(cal<0)//verifico que el numero ingresado  no sea menor a cero osea negativa
							fin=0;//si es menor a cero osea negativa fin lo igualo a cero para terminar el proceso
